...
public class MirrorItem extends Item {
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
        if (destination == Destination.OTHER_PLAYER && !hasAnotherPlayer(player)) {
            player.displayClientMessage(Component.translatable(
                    "item.bountifulbaubles.potion_wormhole.nootherplayers"), true);
    ...
    public ItemStack finishUsingItem(ItemStack stack, Level level, LivingEntity entity) {
        if (entity instanceof ServerPlayer player) {
            boolean toOther = destination == Destination.OTHER_PLAYER
                    || (destination == Destination.SPAWN_OR_OTHER_WHEN_CROUCHING
                    && player.isCrouching());
            boolean teleported = toOther
                    ? TeleportUtil.teleportToAnotherPlayer(player)
    ...
    private boolean hasAnotherPlayer(Player player) {
        return !(player instanceof ServerPlayer serverPlayer)
                || serverPlayer.server.getPlayerList().getPlayerCount() > 1;
    }
