package cursedflames.bountifulbaubles.compat;
...
public final class CurioUtil {
    ...
    public static boolean hasImmunity(Player player, MobEffect effect) {
        return CuriosApi.getCuriosInventory(player).map(handler ->
                !handler.findCurios(stack -> stack.getItem() instanceof BaubleItem bauble
                        && bauble.isImmuneTo(effect)).isEmpty()).orElse(false);
    }
